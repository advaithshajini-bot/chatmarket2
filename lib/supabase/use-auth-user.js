...
export function useAuthUser() {
  ...
  useEffect(() => {
    ...
    const evaluate = async () => {
      ...
      if (!cancelled) {
        // Phase 2 — publicly readable already (RLS: "Profiles are publicly
        // readable"), so this doesn't expose anything new; it just lets
        // the nav correctly show the Admin link only to actual admins
        // instead of to everyone who's logged in. This is a display-only
        // read -- it changes nothing about how admin authorization itself
        // is enforced (that's still private.is_admin()/is_admin_mfa() at
        // the RLS/server layer, untouched).
